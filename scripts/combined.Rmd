...
vcf<-read.table("~/analysis/data/intro_models/chr10_verysmall_haplo.vcf.gz",stringsAsFactors = FALSE) #vcf that has been filtered out to only present one allele call per site per individual
```
...
```{r}
sexscore<-scan("~/analysis/data/intro_models/chr10_verysmall_header.txt",sep='\t',what="character")
#toss sites that happen to have more than one allele
keep<-!grepl(",",vcf[,5]) #none because of the way I've called haplotypes table to check that
