...
ILLUMINACLIP:Computing/Trimmomatic-0.33/adapters/TruSeq3-PE-2.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36 HEADCROP:10
```
...
/home/oziolore/restoreFromData/program/bamtools/bin/bamtools merge \
-list /home/oziolore/restoreFromData/fhet/data/align2/merged_bams2.list -out /home/oziolore/restoreFromData/fhet/data/align2/all_merged2.bam
```
...
```{r}
cov<-read.table("~/fgfh_post/data/angsd/cov_10Mbrand.txt.gz",header=F)
...
ggplot(mpi,
  ...
  scale_fill_manual(values=c("black","black","black","firebrick2","firebrick2","cadetblue3","cadetblue3"))+
  scale_color_manual(values=c(rep("grey40",3),rep("black",4)))+
...
bb<-read.table("~/fgfh_post/data/angsd/raw/BB_neut_1kb.bed",stringsAsFactors=FALSE) #reading in all files
vb<-read.table("~/fgfh_post/data/angsd/raw/VB_neut_1kb.bed",stringsAsFactors=FALSE)
pb<-read.table("~/fgfh_post/data/angsd/raw/PB_neut_1kb.bed",stringsAsFactors=FALSE)
sj<-read.table("~/fgfh_post/data/angsd/raw/SJ_neut_1kb.bed",stringsAsFactors=FALSE)
bnp<-read.table("~/fgfh_post/data/angsd/raw/BNP_neut_1kb.bed",stringsAsFactors=FALSE)
sp<-read.table("~/fgfh_post/data/angsd/raw/SP_neut_1kb.bed",stringsAsFactors=FALSE)
gb<-read.table("~/fgfh_post/data/angsd/raw/GB_neut_1kb.bed",stringsAsFactors=FALSE)
...
u3<-2e-9 #average herring mutation rate

mut<-c(u1,u2,u3)

Ne<-matrix(nrow=8,ncol=3)
rownames(Ne)<-names(pimean)
colnames(Ne)<-c("cichlid u","flounder u","herring u")
  
for(i in 1:3){
  Ne[,i]<-as.numeric(pimean/(4*mut[i]))
}

write.table(Ne[,3],"~/fgfh_post/data/angsd/Ne_estimates.csv",quote=FALSE)
...
jpeg(filename="/Users/eoziolor/Documents/UCD/Projects/Adaptation + Introgression/draft/images/taj.jpeg",width=1000,height=600)
ggplot(mtaj,
       aes(x=variable,y=value,fill=variable,color=variable))+
  geom_violin(trim=FALSE,draw_quantiles = 0.5,lwd=2)+
  scale_fill_manual(values=c("black","black","black","firebrick2","firebrick2","cadetblue3","cadetblue3"))+
...
write.table(allpbs,
            file="~/fgfh_post/data/fst/intermpbs20kb.bed",
            sep = "\t", 
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE)
...
write.table(na.omit(allpbs[all,1:3]),"~/fgfh_post/data/fst/interm_pbs_shared.bed",row.names = FALSE,col.names = FALSE,quote = FALSE,sep='\t')
#source("http://bioconductor.org/biocLite.R")
#biocLite()
library("rtracklayer")

bed1=import("~/fgfh_post/data/fst/intermpbs20kb.bed")

bedall=import("~/fgfh_post/data/fst/interm_pbs_shared.bed")
...
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/review/intermediates.jpeg",width=1200,height=800)
palette(c("grey50","grey90"))
par(mfrow=c(2,1),mar=c(0,3,0,0))
plot(allpbsc[,"sj"],pch=20,cex=3,col=ifelse(allpbs[,"all"]>0,"red",sort(as.factor(allpbs[,1]))),ylim=c(-1.3,1.3),xaxt='n',cex.axis=2)
plot(allpbsc[,"bnp"],pch=20,cex=3,col=ifelse(allpbs[,"all"]>0,"red",sort(as.factor(allpbs[,1]))),ylim=c(-1.3,1.3),xaxt='n',cex.axis=2)
dev.off()
```
...
```{r}
pi<-read.csv("~/fgfh_post/data/angsd/pi",stringsAsFactors=TRUE) #taking only pi values for F. grandis from dataframe
...
```{r}
zpbsn<-read.table("~/fgfh_post/data/dfst/zpbs_20kb",sep='\t',header=FALSE)
zpbs<-zpbsn %>% 
...
pbs_out_temp<-read.table("~/fgfh_post/data/dfst/zregions_max_20kb.bed",stringsAsFactors = FALSE) #loads a pbs vector with windows merged within 50kb of each other and with max and windows count statistics
...
write.table(zpbs[,1:3],"~/fgfh_post/data/dfst/zpbs.bed",row.names = FALSE,col.names = FALSE,quote=FALSE, sep="\t") #saving background genomic regions in bed format
write.table(pbs_out[all,1:3],"~/fgfh_post/data/dfst/pbs_regions_sharedall.bed",row.names = FALSE,col.names = FALSE,quote = FALSE,sep="\t") #saving shared regions to look at overlap with the 1kb regions
write.table(pbs_out[res,1:3],"~/fgfh_post/data/dfst/pbs_regions_sharedres.bed",row.names = FALSE,col.names = FALSE,quote = FALSE,sep="\t")
write.table(pbs_out[interm,1:3],"~/fgfh_post/data/dfst/pbs_regions_sharedinterm.bed",row.names = FALSE,col.names = FALSE,quote = FALSE,sep="\t")
...
bed1=import("~/fgfh_post/data/dfst/zpbs.bed")

bedall=import("~/fgfh_post/data/dfst/pbs_regions_sharedall.bed")
bed1overlall=bed1[bed1 %over% bedall]
hitsall<-findOverlaps(bedall,bed1)
allhit<-subjectHits(hitsall)

bedres=import("~/fgfh_post/data/dfst/pbs_regions_sharedres.bed")
bed1overlres=bed1[bed1 %over% bedres]
hitsres<-findOverlaps(bedres,bed1)
reshit<-subjectHits(hitsres)

bedinterm=import("~/fgfh_post/data/dfst/pbs_regions_sharedinterm.bed")
...
tiff(filename="~/Documents//UCD/Projects/Adaptation + Introgression/review/zpbs.jpeg",width=1200,height=800)

palette(c("grey50","grey90"))
par(mfrow=c(5,1),mar=c(0,3,0,0))
plot(zpbs[,4],pch=20,cex=3,
     col=ifelse(zpbs[,"all"]>0,"purple",sort(as.factor(zpbs[,1]))),
     xlab="",xaxt='n',cex.lab=1,cex.axis=2.2,bty="n",ylim=c(-40,40),yaxs="i")
points(zpbs[,4],pch=20,cex=3,col=ifelse(zpbs[,"all"]>0,"purple",mycol))

plot(zpbs[,5],pch=20,cex=3,
     col=ifelse(zpbs[,"all"]>0,"purple",sort(as.factor(zpbs[,1]))),
     xlab="",xaxt='n',cex.lab=1,cex.axis=2.2,bty="n",ylim=c(-40,40),yaxs="i")
points(zpbs[,5],pch=20,cex=3,col=ifelse(zpbs[,"all"]>0,"purple",mycol))

plot(zpbs[,6],pch=20,cex=3,
     col=ifelse(zpbs[,"all"]>0,"purple",sort(as.factor(zpbs[,1]))),
     xlab="",xaxt='n',cex.lab=1,cex.axis=2.2,bty="n",ylim=c(-40,40),yaxs="i")
points(zpbs[,6],pch=20,cex=3,col=ifelse(zpbs[,"all"]>0,"purple",mycol))

plot(zpbs[,7],pch=20,cex=3,
     col=ifelse(zpbs[,"all"]>0,"purple",sort(as.factor(zpbs[,1]))),
     xlab="",xaxt='n',cex.lab=1,cex.axis=2.2,bty="n",ylim=c(-40,40),yaxs="i")
points(zpbs[,7],pch=20,cex=3,col=ifelse(zpbs[,"all"]>0,"purple",mycol))

plot(zpbs[,8],pch=20,cex=3,
     col=ifelse(zpbs[,"all"]>0,"purple",sort(as.factor(zpbs[,1]))),
     xlab="",xaxt='n',cex.lab=1,cex.axis=2.2,bty="n",ylim=c(-40,40),yaxs="i")
points(zpbs[,8],pch=20,cex=3,col=ifelse(zpbs[,"all"]>0,"purple",mycol))
...
```{r}
PBSout<-read.table("~/fgfh_post/data/fst/hudson_pbsmergeoutliers.bed",stringsAsFactors=FALSE) # Reading in the maximum outlier regions of interest
...
```{r}
PBSout<-read.table("~/fgfh_post/data/fst/hudson_pbsmergeoutliers_max.bed",stringsAsFactors=FALSE) # Reading in the maximum outlier regions of interest
colnames(PBSout)<- c("Scaf","start","end","BBsum", "BBcount","VBsum","VBcount","PBsum","PBcount","SJsum","SJcount","BNPsum","BNPcount") #assigning them column names (we chose columns to be printed earlier)
...
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/review/pbs_sum.jpg",width=1150,height=800)
pbsc$Scaf<-factor(pbsc$Scaf,levels=c("chr1","chr2","chr3","chr4","chr5","chr6","chr7","chr8","chr9","chr10",
                                     "chr11","chr12","chr13","chr14","chr15","chr16","chr17","chr18","chr19",
                                     "chr20","chr21","chr22","chr23","chr24"))
palette(c("grey90","grey50"))
par(mfrow=c(5,1),mar=c(0,4,0,0))
plot(pbsc[,4],pch=20,cex=3,
     col=ifelse(pbsc[,"all"]>0,"purple",as.factor(pbsc[,1])),
     xlab="",xaxt='n',cex.lab=1,cex.axis=4,bty="n",ylim=c(-.5,3.8),yaxs="i",ylab='')
points(pbsc[,4],pch=20,cex=3,col=ifelse(pbsc[,"all"]>0,"purple",mycol))

plot(pbsc[,5],pch=20,cex=3,
     col=ifelse(pbsc[,"all"]>0,"purple", as.factor(pbsc[,1])),
     xlab="",xaxt='n',cex.lab=1,cex.axis=4,bty="n",ylim=c(-.5,3.8),yaxs="i",ylab='')
points(pbsc[,5],pch=20,cex=3,col=ifelse(pbsc[,"all"]>0,"purple",mycol))

plot(pbsc[,6],pch=20,cex=3,
     col=ifelse(pbsc[,"all"]>0,"purple", as.factor(pbsc[,1])),
     xlab="",xaxt='n',cex.lab=1,cex.axis=4,bty="n",ylim=c(-.5,3.8),yaxs="i",ylab='')
points(pbsc[,6],pch=20,cex=3,col=ifelse(pbsc[,"all"]>0,"purple",mycol))

plot(pbsc[,7],pch=20,cex=3,
     col=ifelse(pbsc[,"all"]>0,"purple",as.factor(pbsc[,1])),
     xlab="",xaxt='n',cex.lab=1,cex.axis=4,bty="n",ylim=c(-.5,3.8),yaxs="i",ylab='')
points(pbsc[,7],pch=20,cex=3,col=ifelse(pbsc[,"all"]>0,"purple",mycol))

plot(pbsc[,8],pch=20,cex=3,
     col=ifelse(pbsc[,"all"]>0,"purple",as.factor(pbsc[,1])),
     xlab="",xaxt='n',cex.lab=1,cex.axis=4,bty="n",ylim=c(-.5,3.8),yaxs="i",ylab='')
points(pbsc[,8],pch=20,cex=3,col=ifelse(pbsc[,"all"]>0,"purple",mycol))
dev.off()
```
...
write.table(pbsc[,1:3],"~/fgfh_post/data/fst/PBS_keep_1kb.bed",row.names = FALSE,col.names = FALSE,quote=FALSE,sep='\t')
write.table(na.omit(pbsc[all,1:3]),"~/fgfh_post/data/fst/pbs_regions_sharedall.bed",row.names = FALSE,col.names = FALSE,quote = FALSE,sep='\t')
write.table(na.omit(pbsc[res,1:3]),"~/fgfh_post/data/fst/pbs_regions_sharedres.bed",row.names = FALSE,col.names = FALSE,quote = FALSE,sep='\t')
write.table(na.omit(pbsc[interm,1:3]),"~/fgfh_post/data/fst/pbs_regions_sharedinterm.bed",row.names = FALSE,col.names = FALSE,quote = FALSE,sep='\t')
write.table(na.omit(pbsc[bbu,1:3]),"~/fgfh_post/data/fst/pbs_regions_sharedbbu.bed",row.names = FALSE,col.names = FALSE,quote = FALSE,sep='\t')
write.table(na.omit(pbsc[vbu,1:3]),"~/fgfh_post/data/fst/pbs_regions_sharedvbu.bed",row.names = FALSE,col.names = FALSE,quote = FALSE,sep='\t')
write.table(na.omit(pbsc[pbu,1:3]),"~/fgfh_post/data/fst/pbs_regions_sharedpbu.bed",row.names = FALSE,col.names = FALSE,quote = FALSE,sep='\t')
write.table(na.omit(pbsc[sju,1:3]),"~/fgfh_post/data/fst/pbs_regions_sharedsju.bed",row.names = FALSE,col.names = FALSE,quote = FALSE,sep='\t')
write.table(na.omit(pbsc[bnpu,1:3]),"~/fgfh_post/data/fst/pbs_regions_sharedbnpu.bed",row.names = FALSE,col.names = FALSE,quote = FALSE,sep='\t')
...
bed1=import("~/fgfh_post/data/fst/PBS_keep_1kb.bed")

bedall=import("~/fgfh_post/data/fst/pbs_regions_sharedall.bed")
bed1overlall=bed1[bed1 %over% bedall]
hitsall<-findOverlaps(bedall,bed1)
allhit<-subjectHits(hitsall)
...
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/pbs_interm_random.jpg",width=1000,height=800)
par(mfrow=c(2,3),mar=c(4,4,2,2))
for(i in 1:length(nam)){
  hist(rimeans[,i],main='',breaks=30,xlim=c(range(rimeans[,i],na.rm=TRUE)[[1]]-.01,intermeans[i]+.01),
...
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/review/Ahr1a2a.jpg",width=1000,height=800)
par(mfrow=c(5,1),mar=c(4,6,0,0),mgp=c(1,2,0))
for(i in 1:5){
  plot(subsmooth(pbsc1[1:5000,i+3]),pch=20,cex=.8,ylim=c(0,1.8),bty='l',cex.axis=4,ylab='',xlab='')
  box(bty='l',lwd=3)
  abline(v=c(70,77),col="red",lty=2,lwd=4)
  abline(v=c(79,84),col="purple",lty=2,lwd=4)
}
dev.off()
```
...
library(ape)
library(stringr)
vcf<-read.table("~/fgfh_post/data/fst/ahr1a2a_haplo.vcf.gz",stringsAsFactors = FALSE) #vcf that has been filtered out to only present one allele call per site per individual
...
#plotting tree
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/ahr1a2a_NJ.jpg",width=400,height=1000)
...
#plotting mds
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/ahr1a2a_MDS.jpg",width=1000,height=600)
...
* have to move this to farm cluster since I don't have access to kodiak.

* Selecting the introgressed region with borders chr10:23911854-26893527

```{r}
#chr10 23911854 26893527

pbsc10<-pbsc %>% filter(str_detect(Scaf,"\\bchr10\\b"))

jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/review/Arnt_chr10.jpg",width=1000,height=800)
par(mfrow=c(5,1),mar=c(4,6,0,0),mgp=c(1,2,0))

for(i in 1:5){
  plot(pbsc10[20000:30000,i+3],pch=20,cex=.5,ylim=c(0,.6),bty='l',cex.axis=4,ylab='',xlab='', col="black")
  box(bty='l',lwd=3)
  abline(v=c(3940,3965),col="red",lty=2,lwd=1.5)
}
dev.off()
...
#SBATCH -J arnt_intr
#SBATCH -e arnt_intr-%j.o
#SBATCH -o arnt_intr-%j.o
#SBASTCH -n 8
#SBATCH --mem=16000
#SBATCH -p high
#SBATCH -t 1-00:00
...
my_vcf=/home/eoziolor/fgran/data/varcall/chr10.vcf.bgz
my_list=/home/eoziolor/fgran/data/introgression/arnt_chr10.txt
my_out=/home/eoziolor/fgran/data/introgression/arnt_chr10_fgfh.vcf.bgz

$my_tabix -fh $my_vcf chr10:0-1 | $my_bgz > $my_out
xargs -a $my_list -I {} $my_tabix -f $my_vcf {} | $my_bgz >> $my_out

```

```{bash}
scp -P 2022 farm:/home/eoziolor/fgran/data/introgression/arnt_chr10_fgfh.vcf.bgz ~/fgfh_post/data/fst/
```

* Since the vcf is already haploidized, no need for the following step

```{bash}
#zcat ~/fgfh_post/data/fst/arnt_chr10.vcf.bgz | ~/fgfh_post/scripts/haplo/haploidize_gran_mine.r | gzip > ~/fgfh_post/data/fst/arnt_chr10_haplo.vcf.gz
```

#### Creating new sexscore

```{bash}
zcat < arnt_chr10_fgfh.vcf.bgz | grep -v "##" | head -n 1 | tr '\t' '\n' | sed 's/\./\-/g' | tr '-' '\t' | grep -v "BU" | awk '{print $1}' >> sexscore
zcat < arnt_chr10_fgfh.vcf.bgz | grep -v "##" | head -n 1 | tr '\t' '\n' | sed 's/\./\-/g' | tr '-' '\t' | grep "BU" | awk '{print $2}' >> sexscore
```
...
library(ape)
library(stringr)
vcf<-read.table("~/fgfh_post/data/fst/arnt_chr10_fgfh.vcf.bgz",stringsAsFactors = FALSE) #vcf that has been filtered out to only present one allele call per site per individual
sexscore<-read.table("~/fgfh_post/data/fst/sexscore",header=TRUE)
cname<-c(as.character(sexscore[,1])) # colnames for the vcf
colnames(vcf)<-cname
...
#population identifiers
popcol<-ifelse(grepl("BB",cname),"black",
                      ifelse(grepl("VB",cname),"black",
                             ifelse(grepl("PB",cname),"black",
                                    ifelse(grepl("SJ",cname),"firebrick2",
                                           ifelse(grepl("BNP",cname),"firebrick2",
                                                  ifelse(grepl("SP",cname),"cadetblue3",
                                                         ifelse(grepl("GB",cname),"cadetblue3","orchid1")))))))
popcol2<-ifelse(grepl("BB",cname),"black",
                      ifelse(grepl("VB",cname),"black",
                             ifelse(grepl("PB",cname),"black",
                                    ifelse(grepl("SJ",cname),"firebrick2",
                                           ifelse(grepl("BNP",cname),"firebrick2",
                                                  ifelse(grepl("SP",cname),"cadetblue3",
                                                         ifelse(grepl("GB",cname),"cadetblue1","orchid1")))))))              
...
popname<-c("BB","VB","PB","SJ","BNP","SP","GB","FH")
popnamec<-c("black","black","black","firebrick1","firebrick1","cadetblue3","cadetblue3","orchid1")
#distance matrix and nj tree
d <- t(gt) %>% dist()
#dd<-as.dist(ultrametric(d)) #imputing data for missing values using ultrametric procedure
mds <- cmdscale(d)
...
#plotting tree
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/review/arnt_chr10_NJ.jpg",width=400,height=1000)
plot(tr,"unrooted",show.tip.label=FALSE)
tiplabels(pch=20,col=popcol2,cex=1.2)
dev.off()

#plotting mds
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/review/arnt_chr10_MDS.jpg",width=1000,height=600)
par(mfrow=c(1,1),mar=c(3,3,0,0))
plot(mds,col=popcol2,cex=2,cex.axis=2,pch=20,bty='l')
box(bty='l',lwd=5)
dev.off()
...
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/review/Arnt_chr8.jpg",width=1000,height=800)
par(mfrow=c(5,1),mar=c(4,6,0,0),mgp=c(1,2,0))
for(i in 1:5){
  plot(subsmooth(pbsc8[14000:18000,i+3]),pch=20,cex=.5,ylim=c(0,.5),bty='l',cex.axis=4,ylab='',xlab='')
  box(bty='l',lwd=3)
...
library(ape)
library(stringr)
vcf<-read.table("~/fgfh_post/data/fst/arnt_chr8_haplo.vcf.gz",stringsAsFactors = FALSE) #vcf that has been filtered out to only present one allele call per site per individual
sexscore<-read.table("~/fgfh_post/scripts/depth/sexscore",header=TRUE)
cname<-c(seq(1:9),as.character(sexscore[,1])) # colnames for the vcf
colnames(vcf)<-cname
...
popname<-c("BB","VB","PB","SJ","BNP","SP","GB")
popnamec<-c("black","black","black","firebrick2","firebrick2","cadetblue3","cadetblue3")
#distance matrix and nj tree
d <- t(gt) %>% dist()
#dd<-as.dist(ultrametric(d)) #imputing data for missing values using ultrametric procedure
mds <- cmdscale(d)
...
#plotting tree
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/arnt_chr8_NJ.jpg",width=400,height=1000)
plot(tr,"unrooted",show.tip.label=FALSE)
tiplabels(pch=21,col=popcol2,bg=popfill2,cex=1.2)
legend("topright",pch=20,cex=1.2,legend=popname,col=popnamec)
dev.off()
...
#plotting mds
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/arnt_chr8_MDS.jpg",width=1000,height=600)
par(mfrow=c(1,1),mar=c(3,3,0,0))
plot(mds,col=popcol2,cex=2,cex.axis=2,pch=21,
     bg=popfill2,bty='l')
box(bty='l',lwd=5)
...
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/review/Arnt_chr2.jpg",width=1000,height=800)
par(mfrow=c(5,1),mar=c(4,6,0,0),mgp=c(1,2,0))

for(i in 1:5){
  plot(subsmooth(pbsc2[23000:26000,i+3]),pch=20,cex=.5,ylim=c(0,.4),bty='l',cex.axis=4,ylab='',xlab='')
  box(bty='l',lwd=3)
...
library(ape)
library(stringr)
vcf<-read.table("~/fgfh_post/data/fst/aip_chr2_haplo.vcf.gz",stringsAsFactors = FALSE) #vcf that has been filtered out to only present one allele call per site per individual
sexscore<-read.table("~/fgfh_post/scripts/depth/sexscore",header=TRUE)
cname<-c(seq(1:9),as.character(sexscore[,1])) # colnames for the vcf
colnames(vcf)<-cname
...
#plotting tree
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/aip_chr2_NJ.jpg",width=400,height=1000)
plot(tr,"unrooted",show.tip.label=FALSE)
tiplabels(pch=21,col=popcol2,bg=popfill2,cex=1.2)
legend("topright",pch=20,cex=1.2,legend=popname,col=popnamec)
dev.off()
...
#plotting mds
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/aip_chr2_MDS.jpg",width=1000,height=600)
par(mfrow=c(1,1),mar=c(3,3,0,0))
plot(mds,col=popcol2,cex=2,cex.axis=2,pch=21,
     bg=popfill2,bty='l')
box(bty='l',lwd=5)
...
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/aqp3.jpg",width=1000,height=800)
par(mfrow=c(5,1),mar=c(3,3,0,0),mgp=c(1,1,0))
for(i in 1:5){
  plot(pbsc24[25000:27000,i+3],pch=20,cex=.5,ylim=c(0,1),bty='l',cex.axis=2,ylab='',xlab='')
  box(bty='l',lwd=3)
  abline(v=c(1099:1103),col="red",lty=2,lwd=1.5)
}
dev.off()
...
my_vcf=/data/oziolore/fhet/data/varcall/filtered_chr.vcf.bgz
my_list=/data/oziolore/fhet/data/outliers/aqp_chr24.txt
my_out=/data/oziolore/fhet/data/outliers/aqp_chr24.vcf.bgz
...
```{bash}
scp kodiak:/data/oziolore/fhet/data/outliers/aqp_chr24.vcf.bgz ~/fgfh_post/data/fst/
```

```{bash}
zcat < ~/fgfh_post/data/fst/aqp_chr24.vcf.bgz | ~/fgfh_post/scripts/haplo/haploidize_gran_mine.r | gzip > ~/fgfh_post/data/fst/aqp_chr24_haplo.vcf.gz
```
...
library(ape)
library(stringr)
vcf<-read.table("~/fgfh_post/data/fst/aqp_chr24_haplo.vcf.gz",stringsAsFactors = FALSE) #vcf that has been filtered out to only present one allele call per site per individual
sexscore<-read.table("~/fgfh_post/scripts/depth/sexscore",header=TRUE)
cname<-c(seq(1:9),as.character(sexscore[,1])) # colnames for the vcf
colnames(vcf)<-cname
...
#plotting tree
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/aqp3_NJ.jpg",width=400,height=1000)
plot(tr,"unrooted",show.tip.label=FALSE)
tiplabels(pch=21,col=popcol2,bg=popfill2,cex=1.2)
legend("topright",pch=20,cex=1.2,legend=popname,col=popnamec)
dev.off()
...
#plotting mds
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/aqp3_MDS.jpg",width=1000,height=600)
par(mfrow=c(1,1),mar=c(3,3,0,0))
plot(mds,col=popcol2,cex=2,cex.axis=2,pch=21,
     bg=popfill2,bty='l')
box(bty='l',lwd=5)
...
### MarvelD2 and Occludin
```{r}
#plotting AQP3

pbsc11<-pbsc %>% filter(str_detect(Scaf,"\\bchr11\\b"))

jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/marocc_chr11.jpg",width=1000,height=800)

par(mfrow=c(5,1),mar=c(3,3,0,0),mgp=c(1,1,0))
for(i in 1:5){
  plot(pbsc11[34300:34440,i+3],pch=20,cex=2,ylim=c(0,1),bty='l',cex.axis=2,ylab='',xlab='')
  box(bty='l',lwd=3)
  abline(v=c(43:44),col="red",lty=2,lwd=2)
  abline(v=c(42:43),col="purple",lty=2,lwd=2)
}
...
my_vcf=/data/oziolore/fhet/data/varcall/filtered_chr.vcf.bgz
my_list=/data/oziolore/fhet/data/outliers/marvocc_chr11.txt
my_out=/data/oziolore/fhet/data/outliers/marvocc_chr11.vcf.bgz
...
```{bash}
scp kodiak:/data/oziolore/fhet/data/outliers/marvocc_chr11.vcf.bgz ~/fgfh_post/data/fst/
```

```{bash}
zcat < ~/fgfh_post/data/fst/marvocc_chr11.vcf.bgz | ~/fgfh_post/scripts/haplo/haploidize_gran_mine.r | gzip > ~/fgfh_post/data/fst/marvocc_chr11_haplo.vcf.gz
```
...
library(ape)
library(stringr)
vcf<-read.table("~/fgfh_post/data/fst/marvocc_chr11_haplo.vcf.gz",stringsAsFactors = FALSE) #vcf that has been filtered out to only present one allele call per site per individual
sexscore<-read.table("~/fgfh_post/scripts/depth/sexscore",header=TRUE)
cname<-c(seq(1:9),as.character(sexscore[,1])) # colnames for the vcf
colnames(vcf)<-cname
...
#plotting tree
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/marvocc_NJ.jpg",width=400,height=1000)
plot(tr,"unrooted",show.tip.label=FALSE)
tiplabels(pch=21,col=popcol2,bg=popfill2,cex=1.2)
legend("topright",pch=20,cex=1.2,legend=popname,col=popnamec)
dev.off()
...
#plotting mds
jpeg(filename="~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/marvocc_MDS.jpg",width=1000,height=600)
par(mfrow=c(1,1),mar=c(3,3,0,0))
plot(mds,col=popcol2,cex=2,cex.axis=2,pch=21,
     bg=popfill2,bty='l')
box(bty='l',lwd=5)
...
library(ape)
library(stringr)
vcf<-read.table("~/fgfh_post/data/fst/hudson_out_res_haplo.vcf.bgz",stringsAsFactors = FALSE) #vcf that has been filtered out to only present one allele call per site per individual
sexscore<-read.table("~/fgfh_post/scripts/depth/sexscore",header=TRUE)
cname<-c(seq(1:9),as.character(sexscore[,1])) # colnames for the vcf
colnames(vcf)<-cname
...
library(ape)
library(stringr)
vcf<-read.table("~/fgfh_post/data/fst/hudson_out_all_haplo.vcf.bgz",stringsAsFactors = FALSE) #vcf that has been filtered out to only present one allele call per site per individual
sexscore<-read.table("~/fgfh_post/scripts/depth/sexscore",header=TRUE)
cname<-c(seq(1:9),as.character(sexscore[,1])) # colnames for the vcf
colnames(vcf)<-cname
...
#plotting mds
jpeg(filename="~/Documents//UCD/Projects/Adaptation + Introgression/draft/images/pbs_sharedout.jpg",width=1000,height=400)
par(mfrow=c(1,1),mar=c(4,5,1,1),mgp=c(2,2,5))
plot(mds,col=popcol2,cex=4,cex.axis=4,pch=20,bty='l',xlab='',ylab='')
box(bty='l',lwd=5)
dev.off()
...
cl_mig_stagSweeps = readRDS("~/fgfh_post/data/runDMC/compLikelihood_mig_stagSweeps_AHR.RDS")
cl_neutral = readRDS("~/fgfh_post/data/runDMC/compLikelihood_neutral_AHR.RDS")
cl_sv = readRDS("~/fgfh_post/data/runDMC/compLikelihood_stdVar_AHR.RDS")
...
selsAll_scaled = (sels[1:31] - sels[1]) / (sels[31] - sels[1]) #get all sels and put on (0,1) scale
times20_scaled = (times[1:20] - times[1]) / (times[20] - times[1]) #get last times and put on (0,1) scale

#times21_scaled = (times[1:21] - times[1]) / (times[21] - times[1]) #get last times and put on (0,1) scale

yMin = min(c(maxCL_mig_stagSweeps.sels[1:31], maxCL_sv.sels[1:31]) - cl_neutral)
yMax = max(c(maxCL_mig_stagSweeps.sels[1:31], maxCL_sv.sels[1:31]) - cl_neutral)

jpeg("~/Documents/UCD/Projects/Adaptation + Introgression/review/intro_models.jpeg",width=600,height=400)
par(oma = c(1,1.5,2,1.5),mar=c(3,3,3,3),mgp=c(1,2.5,0))
plot(selsAll_scaled, maxCL_mig_stagSweeps.sels[1:31] - cl_neutral, cex.axis = 3, ylim = c(yMin, yMax), type = "l", col = selColor, xlim = c(0, 1), yaxt = "none", xaxt = "none", xlab = "", ylab =  "", lwd = 6, cex.lab = 2)
box(lwd=6)
lines(selsAll_scaled, maxCL_sv.sels[1:31] - cl_neutral, col = selColor, lwd = 6, lty = 2)
axis(1, at = seq(0, 1, length.out = 5), labels = seq(0, 1, length.out = 5), col = selColor, lwd = 6, cex.axis = 3)
lines(times20_scaled, maxCL_mig_stagSweeps.times[1:20] - cl_neutral, col = timeColor, lwd = 6, lty = 1)
axis(3, lwd = 6, at = seq(0, 1, length.out = 6), labels = seq(times[1], times[20], length.out = 6), col = timeColor, cex.axis = 3)
axis(2, at = (0:5)*10^5, labels = (0:5), cex.axis = 3, las = 1)
#legend("bottomright", lty = c(1,2), legend = c("Introgression", "ILS"), lwd = 3)
#mtext(line = 3, side = 1, text = "Selection coefficient", cex = 1.3)
#mtext(line = 3, side = 3, text = "Generations to introgression", cex = 1.1)
dev.off() 

### Old zoomed in version
maxCL_mig_stagSweeps.sels = sapply(1 : length(sels), function(i) max(unlist(cl_mig_stagSweeps[[i]])))
maxCL_sv.sels = sapply(1 : length(sels), function(i) max(unlist(cl_sv[[i]])))
...
jpeg("~/Documents/UCD/Projects/Adaptation + Introgression/draft/images/intro_models.jpeg",width=600,height=400)
  ...
  plot(sels18_scaled, maxCL_mig_stagSweeps.sels[18:31] - cl_neutral, 
       cex.axis = 1.3, ylab = "", ylim = c(1e5, yMax), 
...
library(gtools)
library('naturalsort')
load("~/fgfh_post/scripts/introgression/dxyscan.Rdata") #loading data into R
...
tiff("~/Documents/UCD/Projects/Adaptation + Introgression/review/introgression_dxy.tiff",width=1400,height=600)
palette(c("grey40","grey80"))
par(mfrow=c(3,1),mar=c(0,4,0,0))
plot(dxy4[,"BB"],pch=20,cex=2,col=ifelse(dxy4[,"BB"]<col[2],"orchid1",as.factor(dxy4[,"chr"])),bty='n',
     xlab="",ylab="",xaxt='n',ylim=c(-.0271,.01),cex.axis=4)
plot(dxy4[,"VB"],pch=20,cex=2,col=ifelse(dxy4[,"VB"]<col[1],"orchid1",as.factor(dxy4[,"chr"])),bty='n',
     xlab="",ylab="",xaxt='n',ylim=c(-.0271,.01),cex.axis=4)
plot(dxy4[,"PB"],pch=20,cex=2,col=ifelse(dxy4[,"PB"]<col[3],"orchid1",as.factor(dxy4[,"chr"])),bty='n',
     xlab="",ylab="",xaxt='n',ylim=c(-.0271,.01),cex.axis=4)
dev.off()
...
write.table(total_chr10,"~/fgfh_post/data/intro_models/chr10_region.txt",col.names = FALSE,row.names = FALSE,quote = FALSE) #writing it down
``` 
...
load("~/fgfh_post/scripts/introgression/AHR_elias_plotdata.Rdata")
names<-row.names(mds)
popcol<-ifelse(grepl("-",names),"orchid1",
...
popcol2<-ifelse(grepl("-",names),"orchid1",
               ifelse(grepl("BB",names),"black",
                      ifelse(grepl("VB",names),"black",
                             ifelse(grepl("PB",names),"black",
                                    ifelse(grepl("SJ",names),"firebrick2",
...
#plotting MDS
tiff("~/Documents/UCD/Projects/Adaptation + Introgression/review/introgression_mds.tiff",width=650,height=600)
par(mgp=c(2,2,5))
plot(mds,col=popcol,cex=2,cex.axis=2.5,pch=as.numeric(popsym),
...
library('ggplot2')
library('reshape')
block<-read.table("~/fgfh_post/data/introgression/grandis_int_blocks.txt",sep='\t',head=TRUE) #reading in introgression blocks
...
tiff("~/Documents/UCD/Projects/Adaptation + Introgression/review/introgression_size.tiff",width=700,height=400)
ggplot(counts2,
...
#  geom_line(aes(col=pop),lwd=3)+
  ...
  theme_classic()+
  scale_color_manual(values=c("black","black","black","firebrick2","firebrick2","cadetblue3","cadetblue3"))+
...
#loading in the file----
setwd("~/Documents/Elias/Baylor/Papers/11. Gradient and Introgression - DRAFT/Deformity and EROD")

cyp1a<-read.csv("~/Documents/Elias/Baylor/Papers/11. Gradient and Introgression - DRAFT/Deformity and EROD/PCB_erod_0.1ppt.csv",header=T)
...
```{r}
mut<-c(2.00E-09, 3.50E-09, 1.20E-08, 1.48E-08, 1.27E-08, 1.13E-08, 1.66E-08, 8.80E-09, 8.14E-09, 3.90E-09, 1.17E-08, 4.60E-09, 3.60E-09, 2.90E-09, 2.80E-09)
