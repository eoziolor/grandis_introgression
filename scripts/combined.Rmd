...
* Figuring out which scaffolds belong to which chromosome so I can take out chr1 for collaborators
```{r}
load("~/analysis/data/comparison/noah_stats.RData") #loading data produced from individual SNP call pi and dxy calculations

ref<-as.data.frame(cbind(lift[,1],fst[,1]))

ref1<-ref %>% 
  filter(V1=="chr1")

chr1scaf<-unique(ref1[,2])

write.table(chr1scaf,"~/analysis/data/introgression/chr1scaf",quote=FALSE,col.names = FALSE,row.names = FALSE)

```

* Filtering out only those scaffolds from the vcf that match chr1
...
```{bash}
#reformatting list of scaffolds

cat chr1scaf | awk '{OFS=""}{s=":"}{p="-"}{print $1,s,0,p}'>chr1scaf_form

#merging with list of lenghts

paste -d '' chr1scaf_form lengths > chr1_list

#Using tabix to take out that vcf
...
xargs -a $list -I {} tabix -fh $joint_vcf {} | bgzip > chr1.vcf.gz
```
