...
a<-ggplot(pi[subz,],
       aes(x=bb,y=gb)) +
  geom_point(size=fsth[subz,"BB.GB"],
...
tiff(filename="~/backup/UCD/Projects/Adaptation + Introgression/draft/images/fst_pi.tiff",width=800,height=1400,units="px")
...
pbs_out_temp<-read.table("~/analysis/data/fst/hudson_pbsmergeoutliers.bed",stringsAsFactors = FALSE) #loads a pbs vector with windows merged within 50kb of each other and with max and windows count statistics
...
jpeg(filename="~/backup/UCD/Projects/Adaptation + Introgression/draft/images/pbs_sum.jpg",width=1000,height=800)
pbsc$Scaf<-factor(pbsc$Scaf,levels=c("chr1","chr2","chr3","chr4","chr5","chr6","chr7","chr8","chr9","chr10",
                                     "chr11","chr12","chr13","chr14","chr15","chr16","chr17","chr18","chr19",
                                     "chr20","chr21","chr22","chr23","chr24"))
...
zcat ~/analysis/data/fst/hudson_out_all_intr.vcf.bgz | ~/analysis/scripts/haplo/haploidize_gran_mine.r | gzip > ~/analysis/data/fst/hudson_out_all_intr_haplo.vcf.bgz
```
...
z<-read.table("~/analysis/data/fst/hudson_pbsmergeoutliers.bed",stringsAsFactors = FALSE) #loads a pbs vector with windows merged within 50kb of each other and with max and windows count statistics
...
tiff("~/backup/UCD/Projects/Adaptation + Introgression/draft/images/outlier_freqs.tiff",width=1000,height=600)
par(mar=c(4,4,2,2))
...
tiff("~/backup/UCD/Projects/Adaptation + Introgression/draft/images/outlier_freqs2.tiff",width=800,height=800)
...
tiff("~/backup/UCD/Projects/Adaptation + Introgression/draft/images/outlier_main.tiff",width=1000,height=400)
efrm<-melt(efr) #putting it in a dataframe more readable by ggplot
colnames(efrm)<-c("pop","chr","freq")
...
axis(3, lwd = 6, at = seq(0, 1, length.out = 6), labels = seq(times[1]+11, times[18]+11, length.out = 6), col = "gold2", cex.axis = 2.8)
...
#colnames(gts)[1] <- "pos"
col<-scan("~/analysis/data/depth/header.txt",sep='\t',what = "character") #grabbing names of all individuals
dep<-read.table("~/analysis/data/depth/NW_012234474.1_550000-800000.depth.txt.gz",stringsAsFactors=FALSE) #grabbing depth vector made by samtools
sexscore0<-col[-c(1:9)] #removing non-individual fields
sexscore<-gsub("SJSP","SJ",sexscore)
names(sexscore)<-sexscore # for counting later
mat<-read.table("~/analysis/data/plink/popcolors.txt",stringsAsFactors=FALSE,sep="\t") #getting colors for later
...
copies<-as.array(table(copies_per_ind[,c(1,2)]) %>% 
                (function(x){x[,c("GB","SP","BNP","SJ","PB","VB","BB")]}) )
copies.prop<-prop.table(copies,2)*100 

tiff("~/backup/UCD/Projects/Adaptation + Introgression/draft/images/deletion_freq.tiff",width=1000,height=400)
par(lwd=3)
...
vcf<-read.table("~/analysis/data/intro_models/chr10_verysmall_haplo2.vcf.gz",stringsAsFactors = FALSE) #vcf that has been filtered out to only present one allele call per site per individual
```

### Get header for vcf
```{bash}
zcat chr10_verysmall.vcf.bgz | grep -v "##" | head -n 1 > chr10_verysmall_header.txt
```
...
intr<-indfr[indfr<0.41] #getting introgressed ind
nonintr<-indfr[indfr>0.41] #getting non-introgressed ind
...
for(i in dim(gt_vbintr)[[1]]){
  freq[i]<-as.numeric(sum(gt_vbintr[i,],na.rm=TRUE))/length(gt_vbintr[i,!is.na(gt_vbintr[i,])])
}
