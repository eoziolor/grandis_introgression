---
title: "divergence_outliers_21Kb_win"
author: "Elias"
date: "May 14, 2018"
...
#Re-do of outlier window pipeline for divergence with Hudson estimator of Fst (this time using 21kb windows)
...
##Calculating Hudson's Fst from pi and dxy (start from here for zstats)
...
##Subtraction of pi and conversion to z statistic (this time let's use the 20kb values)

```{r}
pi<-cbind(fst2[,"BB"],fst2[,"VB"],fst2[,"PB"],fst2[,"SJSP"],fst2[,"BNP"],fst2[,"GB"],fst2[,"SP"]) #taking only pi values for F. grandis from dataframe
pops<-c("BB","VB","PB","SJSP","BNP","GB","SP") #creating a vector of names for our populations
colnames(pi)<-c(pops)

head(val2) # vector of # of SNPs evaluated to come to a summary statistic per window

pib<-pi/val2[,4] #converting these estimates to per base
...
plot(pbsz[subw,"BB"],pch=20,cex=.2,ylim=c(-10,30))
...
# zpbs<-zpbs[ord,] #applying sorted order to our dataset
# 
# zpbs<-read.table("~/analysis/data/dfst/zpbs_20kb",header=FALSE)
# colnames(zpbs)<-c("Scaf","start","end","BB","VB","PB","SJ","BNP")
# 
# zpbs$Scaf<-factor(zpbs$Scaf,levels=c("chr1","chr2","chr3","chr4","chr5","chr6","chr7","chr8","chr9","chr10",
#                                      "chr11","chr12","chr13","chr14","chr15","chr16","chr17","chr18","chr19",
#                                      "chr20","chr21","chr22","chr23","chr24")) #factors are sometimes not sorted together with it, so sorting them separately
# zpbs<-zpbs %>% 
#   filter(str_detect(Scaf,"chr")) #filtering only chromosome mapped regions

#write.table(zpbs,"~/analysis/data/dfst/zpbs_20kb",row.names = FALSE,col.names = FALSE,quote = FALSE,sep="\t") #writing
zpbs<-read.table("~/analysis/data/dfst/zpbs_20kb",header=FALSE)
colnames(zpbs)<-c("Scaf","start","end","BB","VB","PB","SJ","BNP")
zpbs<-zpbs %>% 
  filter(str_detect(Scaf,"chr")) #filtering only chromosome mapped regions
...
awk '$4>2.070793 || $5>3.693761  || $6>2.908474 || $7>2.406610 || $8>2.087560 ' | \
~/program/bedtools2/bin/bedtools merge -i stdin -d 500000 \
-c 4,4,5,5,6,6,7,7,8,8 \
-o max,count,max,count,max,count,max,count,max,count \
...
```{r}
PBSout<-read.table("~/analysis/data/dfst/zregions_max_20kb.bed",stringsAsFactors=FALSE) # Reading in the maximum outlier regions of interest
colnames(PBSout)<- c("Scaf","start","end","BBsum", "BBcount","VBsum","VBcount","PBsum","PBcount","SJsum","SJcount","BNPsum","BNPcount") #assigning them column names (we chose columns to be printed earlier)
